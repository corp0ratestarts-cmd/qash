...
| **Test / Vector** | Unit tests in `lyapunov.rs`: `delta_window_equal_epsilon_does_not_halt` (TH-3a boundary, `δ_window == ε`), `delta_window_epsilon_plus_one_halts` (`δ_window == ε+1`), `phi_halt_triggers_at_threshold` (`Φ_safety == PHI_MAX_SAFE`), `phi_halt_triggers_one_over_threshold` (`Φ_safety == PHI_MAX_SAFE+1`), plus `phi_safety_sums_across_validators` (sum vs max aggregation). Transition tests in `transition.rs` mirror pre-commit behavior via `lyapunov_delta_equal_epsilon_does_not_halt`, `lyapunov_delta_one_over_epsilon_halts`, `phi_safety_halts_at_threshold_before_commit`, and `phi_safety_halts_one_over_threshold_before_commit`. |
| **Proof** | `proofs/contractivity/lyapunov_stability.v` proves TH-3a (`δ_window ≤ ε ⟹ no halt`) and TH-3b (`halt_triggered ↔ δ_window > ε`) with `epsilon = 20_000`; this is compiled by `.github/workflows/ci.yml` `proofs` job after CI rejects active `Admitted`/`admit` markers and checks axiom coverage. |
| **Status** | ⚠️ |

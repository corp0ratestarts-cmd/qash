...
| **Test / Vector** | Module-level tests exist in `crates/consensus/src/fixed_point.rs`; `tests/vectors/vectors.v1.json` includes a fixed-point case run by `crates/vector-runner`. |
| **Proof** | — |
| **Status** | ⚠️ |
...
| **Test / Vector** | `tests/vectors/vectors.v1.json` includes a leaf-index vector run by `crates/vector-runner`. |

...
QASH's first funding-oriented MVP should be an **offline critical-infrastructure incident-log attestation** demonstrator.
...
|-----------|--------|-------|
| MVP demonstrator | **Planned** | Offline incident-log receipt commitment chosen as the first fundable demonstrator. `TX-MVP-ReceiptCommit` remains Domain B-only and avoids payment, energy settlement, and credential-handling claims. |

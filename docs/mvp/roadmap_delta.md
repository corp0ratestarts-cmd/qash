...
```text
Offline incident-log attestation
```
...
- avoids live credential handling;
- needs only fixed-size commitment records, not a full public payload model;
- maps cleanly to cyber-resilience and audit funding language;
...
| MVP-4 | `mvp-demo-replay` | local sync + replay report | two workspaces converge to same root |
| MVP-5 | `mvp-demo-replay` | selective one-receipt disclosure | unrelated receipts remain private |
| MVP-6 | `mvp-demo-artifacts` | scripted demo + funding evidence bundle | clean checkout demo and bounded claims register |
...
5. produce an evidence bundle for grant applications;

...
fn cavp_json_sha3_256_vectors_match_library() {
    let json_path = concat!(env!("CARGO_MANIFEST_DIR"), "/../../tests/cavp/sha3_256.json");
...
fn verify_sha3_vectors(json: &str) {
    ...
    assert_eq!(empty_got.as_ref(), empty_expected.as_slice(), "SHA3-256 TC1 (empty) mismatch");
    assert!(json.contains("a7ffc6f8bf1ed76651c14756a061d662f580ff4de43b49fa82d80a4b80f8434a"),
    ...
    assert_eq!(abc_got.as_ref(), abc_expected.as_slice(), "SHA3-256 TC2 ('abc') mismatch");
    assert!(json.contains("3a985da74fe225b2045c172d6bd390bd855f086e3e9d525b46bfe24511431532"),
    ...
    assert_eq!(a3_got.as_ref(), a3_expected.as_slice(), "SHA3-256 TC3 (0xa3*200) mismatch");
    assert!(json.contains("79f38adec5c20307a98ef76e8324afbfd46cfd81b22e3973c65fa1bd9de31787"),
...
fn cavp_json_hmac_sha256_vectors_match_library() {
    let json_path = concat!(env!("CARGO_MANIFEST_DIR"), "/../../tests/cavp/hmac_sha256.json");
    ...
    assert_eq!(tc1_got.as_ref(), tc1_exp.as_slice(), "HMAC-SHA-256 TC1 mismatch");
    assert!(raw.contains("b0344c61d8db38535ca8afceaf0bf12b881dc200c9833da726e9376c2e32cff7"),
    ...
    assert_eq!(tc2_got.as_ref(), tc2_exp.as_slice(), "HMAC-SHA-256 TC2 mismatch");
    assert!(raw.contains("5bdcc146bf60754e6a042426089575c75a003f089d2739839dec58b964ec3843"),
    ...
    assert_eq!(tc3_got.as_ref(), tc3_exp.as_slice(), "HMAC-SHA-256 TC3 mismatch");
    assert!(raw.contains("773ea91e36800e46854db8ebd09181a72959098b3ef8c122d9635514ced565fe"),

...
impl ContractModelState {
    ...
    fn apply(&mut self, input: &EpochInput) -> Result<[u8; 32], HaltReason> {
        let _ = advance_epoch(&mut self.runtime, input)?;
...
fn replay_vectors_match_contract_model_and_runtime() {
    ...
    for entry in entries {
        ...
        let mut runtime_state = ContractModelState::new(vector.validator_count);
        ...
        for (idx, step) in vector.steps.iter().enumerate() {
            ...
            let runtime_res = runtime_state.apply(&input);
            ...
            match (runtime_res, model_res, expected_halt) {
                (Ok(runtime_fp), Ok(model_fp), HaltReason::None) => {
                    ...
                    assert_eq!(runtime_state.runtime.halt_reason, HaltReason::None);
                ...
                (Err(runtime_h), Err(model_h), expected) => {
                    ...
                    assert_eq!(runtime_state.runtime.halt_reason, expected);
                    assert_eq!(model_state.runtime.halt_reason, expected);
                    assert_eq!(
                        state_fingerprint(&runtime_state.runtime),
            ...
            assert_eq!(runtime_state.runtime.epoch, model_state.runtime.epoch);
            assert_eq!(
                runtime_state.runtime.convergence_window.raw_parts(),

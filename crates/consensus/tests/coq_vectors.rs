...
fn run_tv0(vc: u32) -> EpochState { genesis(vc) }

fn run_idle_epochs(vc: u32, n: usize) -> EpochState {
    let mut s = genesis(vc);
    for _ in 0..n { advance_epoch(&mut s, &idle_input(vc), &[]).unwrap(); }
    s
}
...
fn run_window_fill_then_spike(vc: u32) -> EpochState {
    ...
    let mut s = genesis(vc);
    for _ in 0..WINDOW_SIZE { advance_epoch(&mut s, &idle_input(vc), &[]).unwrap(); }
...
fn run_halt_absorbing(vc: u32) -> EpochState {
    ...
    for _ in 0..5 { let _ = advance_epoch(&mut s, &idle_input(vc), &[]); }
    s
}

fn run_decode_invalid_update_count(vc: u32) -> EpochState {
    let mut s = genesis(vc);
    let bad = EpochInput { updates: [None; MAX_VALIDATORS], update_count: vc - 1 };
...
fn assert_state(state: &EpochState, exp: &Expected) {
    assert_eq!(
        ...
        "{}: epoch mismatch (got {}, want {})", exp.id, state.epoch, exp.epoch
    );
    let halted = state.halt_reason != HaltReason::None;
    assert_eq!(
        halted, exp.halt,
        "{}: halt flag mismatch", exp.id
    );
    assert_eq!(
        state.halt_reason as u8, exp.halt_reason,
        "{}: halt_reason mismatch", exp.id
    );
    assert_eq!(
        state.state_root, exp.state_root,
        "{}: state_root mismatch\n  got:  {:?}\n  want: {:?}", exp.id, state.state_root, exp.state_root
    );
    if let Some(seed) = exp.entropy_seed {
        assert_eq!(state.entropy_seed, seed, "{}: entropy_seed mismatch", exp.id);
    }
    if let Some(nonzero) = exp.seed_nonzero {
        assert_eq!(state.entropy_seed != [0u8; 32], nonzero, "{}: seed_nonzero mismatch", exp.id);
    }
}
...
fn coq_model_parity() {
    ...
    for raw in &parts {
        ...
        let id = format!("TV-{}", &raw[..id_end].trim_matches(|c: char| c == 'T' || c == 'V' || c == '-' || c.is_ascii_digit()));
        ...
        let exp = Expected {
            id: id_val.clone(), desc,
            epoch, halt, halt_reason: halt_reason_from_u8(halt_reason) as u8,
            state_root, entropy_seed, seed_nonzero,
        };

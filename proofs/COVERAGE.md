...
| AX2-refinement | Coq ↔ Rust observational equivalence | Axiom `AX2_rust_refinement` in `model/RefinementStatement.v`; backed by automated Rocq ↔ Rust conformance harness (1,000+ random properties). Strengthen by adding more cases or embedding Rust semantics in Coq (RustBelt / K-Rust, post-v1.1). |

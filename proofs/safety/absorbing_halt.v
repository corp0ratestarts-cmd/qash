(** * QASH — Absorbing Halt Safety Proofs (TH-4, TH-5, TH-6, TH-8)
    ...
    AX-3  Not used in this file directly; TH-8 partial result stated as lemma
          that composes with state_root_injective from encode_injectivity.v
...
Proof. unfold INT_MAX. omega. Qed.

Lemma n_max_pos : 0 < N_max.
Proof. unfold N_max. omega. Qed.

Lemma gamma_pos : 0 < gamma.
Proof. unfold gamma. omega. Qed.
...
Proof.
  ...
  apply Z.lt_mul_diag_r; [apply phi_max_pos | omega].
Qed.
...
Definition sigma_update (current increment : Z) : Z :=
  ...
  if sum >? INT_MAX then INT_MAX else sum.
...
Proof.
  intros c i Hc Hi.
  unfold sigma_update.
  destruct (c + i >? INT_MAX) eqn:H.
  - unfold INT_MAX. omega.
  - apply Z.gtb_ltb in H. apply Z.ltb_ge in H. omega.
Qed.
...
Proof.
  ...
  unfold sigma_update.
  destruct (c + i >? INT_MAX) eqn:H.
  - (* sum > INT_MAX, so result = INT_MAX *)
    apply Z.gtb_ltb in H.
    (* need: c ≤ INT_MAX *)
    (* since c ≥ 0 and increment ≥ 0, c ≤ c + i > INT_MAX
       but we only know c + i > INT_MAX, not c > INT_MAX.
       However c ≤ c + i (since i ≥ 0) and INT_MAX < c + i,
       so if c > INT_MAX we already had an invariant violation.
       The admissibility invariant is: 0 ≤ c ≤ INT_MAX. *)
    (* We add the admissibility precondition *)
    omega.
...
Proof.
  ...
  unfold sigma_update.
  destruct (c + i >? INT_MAX) eqn:H.
  - split; [unfold INT_MAX; omega | unfold INT_MAX; omega].
  - apply Z.gtb_ltb in H. apply Z.ltb_ge in H.
    split; omega.
Qed.
...
(** TH-8 requires:
    ...
    encode_injectivity.v as state_root_injective, which we import here
    as an axiom to avoid circular dependencies. *)

Axiom state_root_injective_import :
  forall s1 s2 : SafetyState,
    (** This is state_root_injective from encode_injectivity.v,
...
(** The full TH-8 statement — depends on encode_injectivity.v:
    ...
    This follows from state_root_injective in encode_injectivity.v,

...
Definition gamma      : Z := 200_000.    (** weight_slash_Sigma — spec §1 *)
...
(** The canonical Σ_i,t update rule.
    ...
    Uses <=? (not >?) so proofs can use Z.ltb_spec without Z.gtb_ltb. *)
...
(** TH-8 requires:
    ...
    Note: state_root_injective_import is intentionally stated as a dependency
    placeholder. It cannot be proved without the full state encoding from
    encode_injectivity.v; the composition is deferred to the integration
    proof file (TH-8 full). *)

Axiom state_root_injective_import :
  forall s1 s2 : SafetyState,
    (** Placeholder dependency: the real import composes with
        TH1_encode_state_injective from encode_injectivity.v.
        Full proof deferred to integration proof file. *)
    s1 = s2.

...
for target in "${TARGETS[@]}"; do
  ...
  case "$target" in
    x86_64-unknown-linux-gnu) "$bin" --vectors "$VECTORS" --out "$out" ;;
    aarch64-unknown-linux-gnu) qemu-aarch64 "$bin" --vectors "$VECTORS" --out "$out" ;;
    riscv64gc-unknown-linux-gnu) qemu-riscv64 "$bin" --vectors "$VECTORS" --out "$out" ;;
  esac
